...
var logic = __webpack_require__(13);
var store = __webpack_require__(1);
...
var onGetGames = function onGetGames(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.index(data).then(ui.onSuccess).catch(ui.onError);
};
...
var displayX = function displayX(target) {
  $(target).text('X').css('color', 'blue');
  store.clickCounter += 1;
  var index = $(target).attr('id'); // 0, 1, - 8
  var gameOver = whoWon();
...
var displayO = function displayO(target) {
  $(target).text('O');
  store.clickCounter += 1;
  var index = $(target).attr('id'); // 0, 1, - 8
  var gameOver = whoWon();
...
var displayLetter = function displayLetter(event) {
  event.preventDefault();
